...
const PERSIAPAN = [
  'Buat project di Google Cloud Console, lalu aktifkan API: My Business Account Management, My Business Business Information, dan Business Profile Performance.',
  'Ajukan akses Google Business Profile API lewat formulir permintaan akses Google — project Anda ditinjau manual dan inilah yang paling lama menunggu.',
  'Akses ULASAN memakai API lama (v4) yang perlu diaktifkan terpisah setelah project disetujui. Wajar bila cek Ulasan merah lebih dulu daripada yang lain.',
  'Buat OAuth Client ID (tipe Web application) + Client Secret di menu Credentials.',
  'Jalankan persetujuan OAuth memakai akun yang mengelola listing RKZ, dengan scope business.manage dan akses offline, lalu simpan refresh token yang diterbitkan.',
  'Isi Account ID (accounts/…) dan Lokasi Utama (locations/…) — atau kosongkan dan salin dari hasil probe.',
]

export default function GoogleBisnisDiagnostik({ slug }: { slug: string }) {
  ...
  async function jalankan() {
    ...
    try {
      const res  = await fetch(`/api/${slug}/pengaturan/google-bisnis/probe`, { method: 'POST' })
  ...
  return (
    <div style={kartu}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap', marginBottom: 8 }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--c-primary)' }}>🔎 Probe Akses Google Business</div>
          <p style={{ fontSize: 13, color: 'var(--c-text-muted)', margin: '4px 0 0', maxWidth: 580, lineHeight: 1.6 }}>
            Cek satu per satu apakah kredensial dan izin sudah cukup untuk menarik daftar lokasi, performa, dan
            ulasan — sebelum fiturnya dibangun. Aman: hanya membaca, tidak mengubah apa pun di Google.
          </p>
        </div>
      ...
      {hasil && (
        <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <p style={{ fontSize: 11, color: 'var(--c-text-faint)', margin: '6px 0 0', lineHeight: 1.6 }}>
            Klik baris yang punya rincian untuk melihat responsnya — di situ Account ID dan Lokasi bisa Anda salin
            ke form di atas.
          </p>
        </div>
      )}
