/**
 * Mesin pencarian segmen — filter gabungan kunjungan SIMRS, kegiatan, dan atribut
 * pasien. Ditempatkan di lib, BUKAN di berkas route: Next.js hanya mengizinkan
 * berkas route mengekspor handler HTTP, dan mengekspor fungsi lain dari sana
 * melanggar konvensi itu (terdeteksi typecheck lewat .next/types). Lagipula
 * pemakainya lebih dari satu — route search, route refresh segmen, dan tool AI
 * Partner — sehingga memang milik lib bersama.
 */
import { z } from 'zod'

export const segmenSearchSchema = z.object({
...
export type SegmenSearchInput = z.infer<typeof segmenSearchSchema>
